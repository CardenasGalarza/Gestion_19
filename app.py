        ...
        if example == 1:
            ...
            with st.sidebar:
                selected = option_menu(
                    ...
                    options=["CargarDt", "Projects", "Contact", "+Simple", "Coordenadas"],  # required
...
##########
                            ...
                            try:
                                if st.button("📞No se ubica cliente"):
                                    ...
                                    if mens > '0':
                                        ...
                                        st.markdown(f'<p class="big-font"; style="text-align:center;background-image: linear-gradient(to right,MistyRose, MistyRose);color:BLACK;font-size:18px;border-radius:2%;"><b>Primero tienes que enviar SMS⚠️</b></p>', unsafe_allow_html=True)
                            except Error as e:
                                print('Gian', e)
    ...
    xs = ['Cardenas', 'LLLERENAL', 'Hinostroza']
    bs = (username in xs)
    if selected == "Coordenadas":
        if bs == True:
            st.markdown(f'<p class="big-font"; style="text-align:center;background-image: linear-gradient(to right,white, white);color:navy;font-size:24px;border-radius:2%;"><b>COORDENADAS GESTION</b></p>', unsafe_allow_html=True)

            with st.form(key='my_form', clear_on_submit=True):
                age = st.slider('Cuántos metros?', 0, 600, 100)
                st.write("Total ", age, 'metros')
